...
var BASE_PACKAGE = {
  ...
  readme: 'README.md',
  dependencies: {
    'hypocore-lib': '^' + bitcore.version,
    'hypocore-node': version
  }
};
...
function createConfigDirectory(options, configDir, isGlobal, done) {
  mkdirp(configDir, function(err) {
    ...
    try {
      fs.writeFileSync(configDir + '/hypocore-node.json', configJSON);
      if (!isGlobal) {
...
function create(options, done) {
  ...
  async.series([
    function(next) {
      // Setup the the hypocore-node directory and configuration
